...
//    CGFloat itemWH = (self.view.tz_width - (self.columnNumber + 1) * margin) / self.columnNumber;
    ...
    if (self.navigationController.navigationBar.isTranslucent) {
        top = 44;
        if (iOS7Later) top += 20;
        collectionViewHeight = tzImagePickerVc.showSelectBtn ? self.view.tz_height - 42 - top : self.view.tz_height - top;;
    } else {
        CGFloat navigationHeight = 44;
        if (iOS7Later) navigationHeight += 20;
        collectionViewHeight = tzImagePickerVc.showSelectBtn ? self.view.tz_height - 42 - navigationHeight : self.view.tz_height - navigationHeight;
    }
...
- (void)configBottomToolBar {
    ...
    CGFloat yOffset = 0;
    if (self.navigationController.navigationBar.isTranslucent) {
        yOffset = self.view.tz_height - 42;//50->42
    } else {
        CGFloat navigationHeight = 44;
        if (iOS7Later) navigationHeight += 20;
        yOffset = self.view.tz_height - 42 - navigationHeight;//50->42
    }
    
    UIView *bottomToolBar = [[UIView alloc] initWithFrame:CGRectMake(0, yOffset, self.view.tz_width, 42)];// 50->42
//    CGFloat rgb = 253 / 255.0;
    bottomToolBar.backgroundColor = [UIColor whiteColor];//[UIColor colorWithRed:rgb green:rgb blue:rgb alpha:1.0];
    
    CGFloat previewWidth = [tzImagePickerVc.previewBtnTitleStr boundingRectWithSize:CGSizeMake(CGFLOAT_MAX, CGFLOAT_MAX) options:NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:15]} context:nil].size.width + 2; //16->15
    if (!tzImagePickerVc.allowPreview) {
        previewWidth = 0.0;
    }
    _previewButton = [UIButton buttonWithType:UIButtonTypeCustom];
    _previewButton.frame = CGRectMake(15, 7, 55, 28);//CGRectMake(10, 3, previewWidth, 44);
    CGFloat largeWidth = (previewWidth > 55) ? previewWidth : 55;
    _previewButton.tz_width = !tzImagePickerVc.showSelectBtn ? 0 : largeWidth;
    [_previewButton setBackgroundImage:[UIImage imageNamedFromMyBundle:@"photo_preview_btn"] forState:UIControlStateNormal];   // add by yj
    [_previewButton addTarget:self action:@selector(previewButtonClick) forControlEvents:UIControlEventTouchUpInside];
    _previewButton.titleLabel.font = [UIFont systemFontOfSize:15]; //[UIFont systemFontOfSize:16];
...
//    [_previewButton setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    [_previewButton setTitleColor:UIColorFromRGB(0x525252) forState:UIControlStateNormal];
    ...
    */
    _doneButton = [UIButton buttonWithType:UIButtonTypeCustom];
    _doneButton.frame = CGRectMake(self.view.tz_width -85-15, 7, 85, 28);
    _doneButton.titleLabel.font = [UIFont systemFontOfSize:15];
    [_doneButton setBackgroundImage:[UIImage imageNamedFromMyBundle:@"photo_sure_photoPickerVc"] forState:UIControlStateNormal];
    [_doneButton addTarget:self action:@selector(doneButtonClick) forControlEvents:UIControlEventTouchUpInside];
    NSString * sureStr = [NSString stringWithFormat:@"%@(%ld/%ld)", tzImagePickerVc.doneBtnTitleStr, [tzImagePickerVc.selectedAssets count], tzImagePickerVc.maxImagesCount];
    [_doneButton setTitle:sureStr forState:UIControlStateNormal];
    [_doneButton setTitle:sureStr forState:UIControlStateDisabled];
    [_doneButton setTitleColor:tzImagePickerVc.oKButtonTitleColorNormal forState:UIControlStateNormal];
    [_doneButton setTitleColor:tzImagePickerVc.oKButtonTitleColorDisabled forState:UIControlStateDisabled];
    _doneButton.enabled = tzImagePickerVc.selectedModels.count || tzImagePickerVc.alwaysEnableDoneBtn;
    ...
    _numberImageView.hidden = YES;  //add by yj
    _numberLabel.hidden = YES;      //add by yj
    
    UIView *divide = [[UIView alloc] init];
    CGFloat rgb2 = 222 / 255.0;
    divide.backgroundColor = UIColorFromRGB(0xe8e8e8);//[UIColor colorWithRed:rgb2 green:rgb2 blue:rgb2 alpha:1.0];
...
- (void)refreshBottomToolBarStatus {
    ...
    NSString * sureStr = [NSString stringWithFormat:@"%@(%ld/%ld)", tzImagePickerVc.doneBtnTitleStr, [tzImagePickerVc.selectedModels count], tzImagePickerVc.maxImagesCount];
    [_doneButton setTitle:sureStr forState:UIControlStateNormal];
    [_doneButton setTitle:sureStr forState:UIControlStateDisabled];
