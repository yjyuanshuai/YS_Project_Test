...
//    [self configBottomToolBar];   // delete by yj
    self.view.clipsToBounds = YES;
    
    
    // add by  yj
    [self createBarButtonItems];
    self.hideStatusAndNaviBar = NO;
}
...
//    [self refreshNaviBarAndBottomBarState];
    ...
    [self refreshNavigationTitleCurrentIndex:_currentIndex showTitle:_showTitle];
}
    ...
    if (!cell.singleTapGestureBlock) {
        ...
        cell.singleTapGestureBlock = ^(){
            ...
            if (weakSelf.isHideNaviBar) {
                _hideStatusAndNaviBar = YES;
            }
            else {
                _hideStatusAndNaviBar = NO;
                [weakSelf refreshNavigationTitleCurrentIndex:indexPath.row showTitle:_showTitle];
            }
            [weakSelf showOrHideNavigationAndStatusBar];
        };
    }
...
- (void)setNavBarAppearance:(BOOL)animated
{
    ...
    navBar.tintColor = iOS7Later ?[UIColor whiteColor] : nil;
...
- (void)createBarButtonItems
{
    ...
    _backButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 60, 41 / 2)];
    _backButton.backgroundColor = [UIColor clearColor];
    [_backButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [_backButton setTitleColor:[UIColor lightGrayColor] forState:UIControlStateHighlighted];
    _backButton.contentHorizontalAlignment = UIControlContentHorizontalAlignmentLeft;
    [_backButton addTarget:self action:@selector(backButtonClick) forControlEvents:UIControlEventTouchUpInside];
    UIBarButtonItem *backBtnItem = [[UIBarButtonItem alloc] initWithCustomView:_backButton];
    self.navigationItem.leftBarButtonItem = backBtnItem;
}

- (void)showOrHideNavigationAndStatusBar
{
    [self.navigationController setNavigationBarHidden:_hideStatusAndNaviBar animated:YES];
    [[UIApplication sharedApplication] setStatusBarHidden:_hideStatusAndNaviBar withAnimation:UIStatusBarAnimationFade];
    _hideStatusAndNaviBar = !_hideStatusAndNaviBar;
}

- (void)refreshNavigationTitleCurrentIndex:(NSInteger)currentIndex showTitle:(BOOL)showTitle
{
    if (showTitle) {
        dispatch_async(dispatch_get_main_queue(), ^{
            self.title = [NSString stringWithFormat:@"%ld/%ld", currentIndex+1, _models.count];
        });
    }
